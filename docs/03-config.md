...
但有它就比没它强 - 训练时一次数值溢出就可能让 loss 突然变成 NaN，整个 run 报废。软裁剪是廉价保险。
...
但 **MoE 不能节省内存**：314B 全部参数都得加载到 GPU，HBM 带宽吃满。MoE 节省的是**算力**和**通信**（如果 expert 分片得当）。
...
这就是为什么 Grok-1 推理"再快也快不到 Mixtral 8x7B 那种水平" - 激活参数从 13B 涨到 86B，内存带宽需求涨 6.6 倍，单 token 时延天然变长。

## 3.7 `MHAttentionConfig`？没这个东西

题目里提到 `MHAttentionConfig`，事实是 Grok-1 没有这个 dataclass。`MultiHeadAttention` 的所有超参由 `TransformerConfig` 直接传入。这一节就是要澄清这件事：**Grok-1 的配置只有 `LanguageModelConfig` 和 `TransformerConfig` 两层**，MoE / 路由 / Attention 的字段都嵌在 `TransformerConfig` 里。
