...
- **稠密派**：Meta 的 LLaMA-2 在 2023 年 7 月发布，提供 7B、13B、70B 三个尺寸，其中 70B 在 2023 年下半年到 2024 年初是工业界用得最多的开源 dense 模型，HuggingFace 上衍生模型上千个。2024 年 2 月 21 日，Google 又开源了 Gemma 7B / 2B 两个尺寸，依然走稠密路线，没有跟随 MoE 潮流。也就是说，2024 年初最有影响力的开源稠密线仍由 Meta 和 Google 把持，参数规模上限在 70B 一档
...
- **MoE 派的开端**：2023 年 12 月 8 日，Mistral AI 在 X 上贴出一条磁力链，没有 README、没有博客、没有任何说明文字，社区下载完才发现里面是 Mixtral 8x7B - 47B 总参、13B 激活、8 个专家每次挑 2 个的 MoE 架构。几天之内陆续跑出的基准让人意外：Mixtral 在 MMLU、HellaSwag、ARC、GSM8K 这些 base 评测上几乎全面胜过 Llama-2 70B，激活参却只有它的不到五分之一。这是 MoE 第一次在公开模型里把"同等激活下更聪明、同等总参下更省算力"这件事正面证明出来，也给了 xAI 三个月后用同款磁力链方式放 Grok-1 一个直接的先例
- **闭源的大型模型**：到 2024 年第一季度，GPT-4、Claude 3、Gemini 1.5 都已经发布，业界根据它们的推理速度、token 价格、长上下文表现等间接信号广泛猜测它们采用了 MoE 架构，但三家厂商都没有正面承认任何架构细节。也就是说，MoE 在闭源前沿模型里的地位只能从外部推断，开源侧需要自己拿出一份完整的 MoE 实现才能让外界把这些猜测落到代码层面
...
- 当时全世界能拿出 8 张 80GB GPU 组成的单机服务器、把这种模型跑起来做推理的研究团队，估算不超过两千个。这是"读懂 Grok-1 推理代码、能在自己机器上验证"的硬件门槛
- 当时全世界能拿出 64 张 80GB GPU 组成的集群、对这种规模的 base 模型做指令微调（SFT）的团队，估算不超过一百个。这是"把 Grok-1 改造成可用对话模型"的工程门槛
...
但 Grok-1 把"显存"这一面压到了极限：314B 参数即便经过 8-bit 量化也仍然需要约 314GB 显存来存放权重本身，单机 8 卡 H100 80GB 总显存是 640GB，扣掉权重后留给 KV cache 和中间激活的余地只剩 100GB 左右。这是 Grok-1 推理硬件门槛之所以从 8 卡起跳的根本原因 - 限制来自总参 314B，而不是激活参 86B；想用更少的卡跑起来，权重直接放不下。
...
1. **指令微调（SFT）权重** - Grok-1 base 模型在直接对话场景下输出质量较差，即便配合精心设计的 prompt 工程也难以补救
2. **RLHF/DPO** 后的版本
3. **训练代码 / 数据 / 词表统计**
...
xAI 开源 base 而不开源后续步骤，相当于把"原料"提供出来，但"配方和工艺"自己保留。这是商业意义上的最优策略 - 既能获得"开源"的公关效果，又不影响产品差异化。OpenAI、Anthropic、Google 从不公开任何 base 模型，xAI 至少公开了 base，已经走得比这三家更远。但相比 Meta 公开 LLaMA-Chat、Mistral 公开 Mixtral-Instruct，xAI 的开源又走得没那么远。
...
JAX 与 PyTorch 在心智模型上有几处常见的工程陷阱：
